...
public class UserController {
    ...
    @PostMapping("/signUp")
    public ResponseEntity<PostSignUpUserDtoRes> signUpUser(@Valid @RequestBody PostCreateUserDtoReq postCreateUserDtoReq) {
    ...
    public ResponseEntity signUpSeller(@Valid @RequestBody PostCreateUserDtoReq postCreateUserDtoReq) {
    ...
    @ApiResponses({
            ...
            @ApiResponse(responseCode = "500",description = "서버 내부 오류")})
    @GetMapping( "/confirm")
    public ResponseEntity confirm(@Valid @ModelAttribute PostEmailConfirmDtoReq req) {
        if (emailVerifyService.verify(req)) {
            userService.updateMemberStatus(req.getEmail());
            return ResponseEntity.ok().body("user email verify success");
        } else {
            return ResponseEntity.badRequest().body("user email verify fail");
        }
    }
